...
export const find =  async (req: Request, res: Response) => {
    ...
    if(typeof lat === "number" && typeof long === "number" && typeof name === "string" && typeof number === "number"&& typeof radius === "number"){
        try {
            const list = await pharmacy1.searchNearbyPharmacies(lat,long,radius,name,number);
            if (list) {
                res.status(200).json({
                  status: "found",
                  data: list,
