...
export class Pharmacies {
      async searchNearbyPharmacies( lat :number, long :number, radius :number,name:string,number:number): Promise<resutls[]> 
      {
        try 
        {
          const latMin = lat - (radius / 111.2);
          const latMax = lat + (radius / 111.2);
          const lonMin = long - (radius / (111.2 * Math.cos(lat * (Math.PI / 180))));
          const lonMax = long + (radius / (111.2 * Math.cos(lat * (Math.PI / 180))));
  ...
  const final_result = responses.filter(response => response.status===specificResult).map(response => ({name:response.name,latitude:response.latitude,longitude:response.longitude}));
